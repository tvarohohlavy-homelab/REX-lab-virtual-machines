...
"@ -split '\s+'
    ...
    if (!$vm) {
        Write-Error "Set-VMKeystrokes | Unable to find VM $VMName"
        return
    }
    ...
    if ($StringInput) {
        ...
        foreach ($token in $tokens) {
            if ($hidCharacterMap.ContainsKey([string]$token)) {
                ...
                Write-Debug "Set-VMKeystrokes | [Input] Character: $($token) -> HIDCode: $($hidCharacterMap[[string]$token].hidCode) -> HIDCodeValue: $($hidCharacterMap[[string]$token].hidCodeValue)"
            } else {
                switch ($token) {
                    ...
                    Default {
                        Write-Error "Set-VMKeystrokes | [KeyInput] The following character `"$($token)`" has not been mapped, you will need to manually process this character"
                    }
                }
            }
        }
